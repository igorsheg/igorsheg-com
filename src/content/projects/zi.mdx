...
<Prescii label="Zi clients converge on one agent session, which owns the provider, journal, and tool catalog"><span class="fg-green">{`interactive   text   JSONL   RPC
     \\         |       |      /
      +--------+-------+-----+
               |
         +-----v------+
...
<Prescii label="The model can call admitted tools directly or orchestrate the same catalog through Code Mode"><span class="fg-blue">{`model <----> AgentSession
                  |
          +-------+-------+
          |               |
