...
Cloudflare Workers Builds runs `pnpm build`, deploys with `pnpm run deploy`, and serves `dist/` through Workers Static Assets. The site is fully static, so it does not use the Cloudflare SSR adapter.
